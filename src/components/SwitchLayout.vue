...
<template>
  <RadioGroup v-model="active" class="flex">
    <div class="inline-flex rounded-md bg-neutral-100 dark:bg-neutral-700 p-1">
      <RadioGroupOption v-for="opt in options" :key="opt.value" :value="opt.value" v-slot="{ checked }" as="template">
        <button :class="[
          'relative inline-flex items-center gap-1.5 rounded-sm px-3 py-1.5 text-sm font-medium transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500',
          checked ? 'bg-white dark:bg-neutral-50 text-zinc-900 ring-1 ring-neutral-100' : 'text-zinc-500 hover:text-zinc-700'
        ]" :title="opt.label">
